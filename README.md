![Python](https://img.shields.io/badge/python-3.10-blue.svg)
![Transformers](https://img.shields.io/badge/Transformers-Hugging%20Face-white.svg)

![Tests](https://github.com/lm-jim/bgc-infiller/actions/workflows/tests.yml/badge.svg)
[![W&B](https://img.shields.io/badge/Weights_&_Biases-Active-gold.svg)](https://wandb.ai/lm-jim-universidad-polit-cnica-de-madrid/bgc-infiller/)
[![Hugging Face Space](https://img.shields.io/badge/%F0%9F%A4%97%20Hugging%20Face-Spaces-yellow)](https://huggingface.co/spaces/lm-jim/mdl-mlops)

---

# Biosynthetic Genomic Cluster (BGC) Infiller

Final Deep Learning Master´s project for Universidad Politécnica de Madrid. A ESM2-based model for infilling BGCs (Biosynthetic Genomic Clusters), aiming to the creation of new possible antibiotics.

## Project content [WIP]

- x
- x
- x
- x

## Deployment instructions [WIP]

### Docker Container

The server can be built as a Docker image using the script `scripts/build-docker.bat` (Windows only) or executing the following command on the project's root folder:

> `docker build -t xxxx .`

Once built, it can be accessed through Docker Desktop or by using the following command:

> `docker run -p 8000:8000 xxxx`

### Online access on HuggingFace Spaces

You can access the online deployment through the following link:

> https://huggingface.co/spaces/lm-jim/bgc-infiller

## Enlaces de interés

| Recurso                   | Enlace                                                                   |
| :------------------------ | :----------------------------------------------------------------------- |
| GitHub Repository         | https://github.com/lm-jim/bgc-infiller                                   |
| Weights & Biases          | https://wandb.ai/lm-jim-universidad-polit-cnica-de-madrid/bgc-infiller/  |
| HuggingFace Space         | https://huggingface.co/spaces/lm-jim/bgc-infiller/                       |
